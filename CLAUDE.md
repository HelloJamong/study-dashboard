...
- **python-dotenv 의존성 제거 예정**: `.env` 기반 설정은 SQLite DB로 마이그레이션 중. 신규 설정 항목은 DB에만 추가할 것.

## 설계 의도

- **STT 엔진**: faster-whisper (CTranslate2 기반, torch 불필요). base 모델이 기본값.
- **요약 엔진**: Gemini API (gemini-2.5-flash 등) 또는 OpenAI API. 키는 DB에서 암호화 로드.
...
`LMS_USER_ID`, `LMS_PASSWORD`, `GOOGLE_API_KEY`, `OPENAI_API_KEY`, `TELEGRAM_BOT_TOKEN`
...
|----|------|------|
| `LMS_USER_ID` | 학번 (암호화) | — |
| `LMS_PASSWORD` | 비밀번호 (암호화) | — |
| `DOWNLOAD_DIR` | 다운로드 경로 (비워두면 자동) | `/data/downloads` |
| `DOWNLOAD_RULE` | 다운로드 규칙 | `video` / `audio` / `both` |
...
| `AI_AGENT` | AI 에이전트 | `gemini` / `openai` |
...
- `.env` — 레거시 설정 파일. **절대 커밋 금지**. DB 마이그레이션 완료 후 삭제 예정
- `.secret_key` — 암호화 키. **절대 커밋 금지**
- `data/` — DB 및 다운로드 파일. **절대 커밋 금지**

**민감 정보 처리**: 학번, 비밀번호, API 키는 TUI 입력 즉시 `crypto.py`로 암호화되어 DB에 저장됨. 평문으로 저장되지 않음.
